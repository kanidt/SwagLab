...
public class OverViewPageSwagvalidation extends BaseClass {
   ...
   System.out.println("product  not successful login ");
	...
	@Test
	public void overviewvalidation() {
		
		try {
			WebElement finishlast = pageManager.getFinishPageSwag().getFinishlast();
			baseclass.clickbutton(finishlast);
		...
		@After
		public void finishsuccessfulvalidation() {
			
			try {
				if (pageManager.getOverViewPageSwag().getSwagLapsText().isDisplayed()) {
					System.out.println("overview is successful login");
					
				} else {
					System.out.println("overview is not successful login");
			...
			try {
				WebElement allIteams = pageManager.getOverViewPageSwag().getAllIteams();
				baseclass.clickbutton(allIteams);
